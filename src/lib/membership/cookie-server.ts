...
import { ACTIVE_MEMBERSHIP_COOKIE } from '@/lib/membership/cookie'
...
export async function getActiveMembershipFromCookies(): Promise<ActiveMembership | null> {
  const cookieStore = await cookies()
  const cookie = cookieStore.get(ACTIVE_MEMBERSHIP_COOKIE)?.value
...
export async function setActiveMembershipInCookies(
  ...
  cookieStore.set(ACTIVE_MEMBERSHIP_COOKIE, value, {
    httpOnly: true,
...
export async function clearActiveMembershipInCookies(): Promise<void> {
  const cookieStore = await cookies()
  cookieStore.set(ACTIVE_MEMBERSHIP_COOKIE, '', {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
    maxAge: 0,
