...
export default async function ManagerInviteStaffPage() {
  ...
  if (!active || active.role !== 'manager') redirect('/select-membership')

  const properties = await getInvitableProperties()
  const allowedRoles = invitableRolesForMembership('manager')
  ...
  return (
    <>
      <PageHeader
        title="Invite staff"
        description="Invite technicians and housekeepers to your property."
      />
